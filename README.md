...
Therefore, this extension _tries_ to use the official (Google-supported) [Go Language Server](https://github.com/golang/tools/blob/master/gopls/README.md). You need to download it and install the binary (named `gopls`) under the `Go.novaextension > Scripts` directory path. It's still in Alpha, but, these days, what isn't?
